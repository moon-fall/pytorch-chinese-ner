...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        precheck_sent = prepare_sequence(valid_data[0][0], word_to_ix)
        precheck_tags = torch.tensor([tag_to_ix[t] for t in valid_data[0][1]], dtype=torch.long)
    ...
    for epoch in range(5):  # again, normally you would NOT do 300 epochs, it is toy data
        ...
        for sentence, tags in train_data:
            ...
            predict = torch.Tensor(model(sentence_in)[1]).long()
            eq = torch.eq(predict,targets)
            acc = eq.sum().item()/eq.numel()
            ...
            train_predict_total=train_predict_total+eq.numel()
            train_predict_correct=train_predict_correct+eq.sum().item()
            train_acc = train_predict_correct/train_predict_total
            
            if(count%100==0):
                print(time.asctime(time.localtime(time.time()))+"    "+"epoch:"+str(epoch)+"    "+"count:"+str(count)+"    "+"train_loss_100_avg:"+format(train_loss.item()/100, '.4f')+"    "+"train_acc_100:"+format(train_acc, '.4f'))
            ...
            if((count>0) & (count%3000==0)):
                ...
                print(time.asctime(time.localtime(time.time()))+"    "+"epoch:"+str(epoch)+"    "+"count:"+str(count)+"    "+"valid_acc_total:"+format(valid_acc_total, '.4f'))
